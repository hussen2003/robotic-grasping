...
class DepthImage(Image):
    ...
    def inpaint(self, missing_value=0):
        ...
        """
        # Add border to handle edges
        self.img = cv2.copyMakeBorder(self.img, 1, 1, 1, 1, cv2.BORDER_DEFAULT)
        
        # Create mask for missing values
        mask = (self.img == missing_value).astype(np.uint8)

        # Convert to float32 and scale to 0-1 for OpenCV
        img_scaled = self.img.astype(np.float32)
        min_val = img_scaled.min()
        max_val = img_scaled.max()
        # Avoid division by zero
        if max_val - min_val > 0:
            img_scaled = (img_scaled - min_val) / (max_val - min_val)
        else:
            img_scaled = img_scaled * 0

        # Inpaint
        img_inpainted = cv2.inpaint(img_scaled, mask, 1, cv2.INPAINT_NS)

        # Rescale back to original depth range
        self.img = img_inpainted * (max_val - min_val) + min_val
